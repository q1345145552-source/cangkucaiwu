...
async def delete_period_payroll(
    ...
    # 已离职/已删除员工的工资单不删（重算不会重新生成，删了就再也补不回来）
    emp_ids = {r.employee_id for r in records}
    emp_flags = {}
    if emp_ids:
        emps = (await db.execute(select(Employee).where(Employee.id.in_(emp_ids)))).scalars().all()
        emp_flags = {e.id: (e.status, bool(e.is_deleted)) for e in emps}

    to_delete = []
    preserved_count = 0
    for r in records:
        st, deleted = emp_flags.get(r.employee_id, (None, False))
        if st == "resigned" or deleted:
            preserved_count += 1
            continue
        to_delete.append(r)
    ...
    return {"message": f"已删除 {period} 的 {len(to_delete)} 条工资记录，保留 {preserved_count} 条离职或已删除员工的记录未参与重算"}
