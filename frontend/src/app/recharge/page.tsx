        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
          <div className="bg-white rounded-xl w-full max-w-md p-6">
            ...
            <div className="space-y-3">
              <div><label className="block text-sm mb-1">客户</label><select className="border rounded px-3 py-2 w-full" value={form.customer_id} onChange={e => setForm({...form, customer_id: +e.target.value})}><option>选择客户</option>{customers.map((c: any) => <option key={c.id} value={c.id}>{c.company_name}</option>)}</select></div>
