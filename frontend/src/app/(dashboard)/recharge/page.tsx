  ...
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <h1 className="page-title">{t("recharge")}</h1>
        <button onClick={() => setShowForm(true)} className="btn-primary">新建申报</button>
      </div>
        ...
        <div className="modal-overlay">
          <div className="bg-white rounded-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              <div><label className="block text-sm mb-1">客户</label><select className="form-input" value={form.customer_id} onChange={e => setForm({...form, customer_id: +e.target.value})}><option value={0}>选择客户</option>{customers.map((c: any) => <option key={c.id} value={c.id}>{c.company_name}</option>)}</select></div>
              <div><label className="block text-sm mb-1">申报日期</label><input type="date" className="form-input" value={form.declare_date} onChange={e => setForm({...form, declare_date: e.target.value})} /></div>
              ...
              <div><label className="block text-sm mb-1">付款方式</label><select className="form-input" value={form.payment_method} onChange={e => setForm({...form, payment_method: e.target.value})}><option value="">选择</option><option value="alipay">支付宝</option><option value="wechat">微信</option><option value="bank_transfer">银行转账</option></select></div>
              <div><label className="block text-sm mb-1">交易单号</label><input className="form-input" value={form.transaction_no} onChange={e => setForm({...form, transaction_no: e.target.value})} /></div>
              ...
              <div>
                ...
                <input type="file" accept="image/png,image/jpeg,image/jpg" onChange={(e) => setSelectedFile(e.target.files?.[0] || null)} className="form-input text-sm" />
